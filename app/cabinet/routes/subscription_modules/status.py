    ...
    if fresh_user.remnawave_uuid:
        try:
            ...
            if stats:
                ...
                changed = False
                if abs((subscription.traffic_used_gb or 0.0) - live_used) > 0.01:
                    subscription.traffic_used_gb = live_used
                    changed = True
                if live_limit and (subscription.traffic_limit_gb or 0) != live_limit:
                    subscription.traffic_limit_gb = live_limit
                    changed = True
                if changed:
                    await db.commit()
